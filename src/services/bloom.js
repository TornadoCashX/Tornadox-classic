...
export class BloomService {
  constructor({ netId, amount, instanceName, fileName, fileFolder }) {
    this.amount = amount
  ...
  async checkBloom(commitment) {
    let cachedBloom = await this.getBloomFromDB()
    if (!cachedBloom) {
      cachedBloom = await this.getBloomFromCache()
    }
    if (!cachedBloom || typeof cachedBloom.test !== 'function') {
      return false
    }
    return cachedBloom.test(commitment)
  }
